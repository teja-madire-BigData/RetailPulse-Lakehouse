...
with DAG(
    dag_id = "silver_sellers",
    description = "Bronze to Silver: SCD Type 1 upsert for sellers",
    schedule = "0 2 * * *",
    ...
    add_step = EmrAddStepsOperator(
        task_id = "add_silver_sellers_step",
    ...
    wait_step = EmrStepSensor(
        task_id = "wait_silver_sellers",
        job_flow_id = CLUSTER_ID,
        step_id = "{{ task_instance.xcom_pull('add_silver_sellers_step')[0] }}",
        aws_conn_id = "aws_default",
        poke_interval = 30,
        timeout = 3600,
