...
with DAG(
    dag_id = "silver_fx_rates",
    description = "Bronze to Silver: processing for fx_rates",
    schedule = "0 2 * * *",
    start_date = pendulum.datetime(2026, 1, 1, tz="UTC"),
    catchup = False,
    default_args = default_args,
    tags = ["silver", "fx_rates"],
) as dag:

    PARTITION_DATE = "{{ (logical_date - macros.timedelta(days=1)).strftime('%Y-%m-%d') }}"

    add_step = EmrAddStepsOperator(
        task_id = "add_silver_fx_rates_step",
        ...
        steps = [{
            ...
            "HadoopJarStep": {
                ...
                "Args": [
                    ...
                    f"{SCRIPTS_PATH}/silver_fx_rates.py",
    ...
    wait_step = EmrStepSensor(
        task_id = "wait_silver_fx_rates",
        job_flow_id = CLUSTER_ID,
        step_id = "{{ task_instance.xcom_pull('add_silver_fx_rates_step')[0] }}",
        aws_conn_id = "aws_default",
        poke_interval = 30,
        timeout = 3600,
